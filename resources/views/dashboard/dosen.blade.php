...
@section('content')
    ...
    <!-- ═══════════════════ JADWAL MINGGUAN ═══════════════════ -->
    <div class="sec" x-data="{ showAdd: false, showEdit: false, editData: {} }">
        <div class="sec-head">
            <div class="sec-title">
            ...
            <button class="btn-add" @click="showAdd = true; addRipple($event, $el)">
                                    ...
                                    <td>
                                        <div class="act-btns">
                                            <button type="button" class="btn-edit"
                                                @click="editData = { id: {{ $j->id }}, hari: '{{ $j->hari }}', jam_mulai: '{{ substr($j->jam_mulai, 0, 5) }}', jam_selesai: '{{ substr($j->jam_selesai, 0, 5) }}', kegiatan: '{{ $j->kegiatan }}', mata_kuliah: '{{ addslashes($j->mata_kuliah) }}', ruangan: '{{ addslashes($j->ruangan) }}', keterangan: '{{ addslashes($j->keterangan) }}' }; showEdit = true"><svg
        ...
        <div x-show="showAdd" x-cloak class="modal-bg" @click.self="showAdd = false">
            <div class="modal" @click.stop>
                <div class="modal-head">
                    <h3><i class="lucide-calendar-range" style="color:#2563eb"></i> Tambah Jadwal Mingguan</h3>
                    <button class="modal-close" @click="showAdd = false"><i class="lucide-x"></i></button>
                </div>
                <form method="POST" action="/jadwal-mingguan">
                    ...
                    <div class="modal-foot">
                        <button type="button" class="btn-cancel" @click="showAdd = false">Batal</button>
        ...
        <!-- Modal Edit Mingguan -->
        <div x-show="showEdit" x-cloak class="modal-bg" @click.self="showEdit = false">
            <div class="modal" @click.stop>
                <div class="modal-head">
                    ...
                    <button class="modal-close" @click="showEdit = false"><i class="lucide-x"></i></button>
                </div>
    ...
    <!-- ═══════════════════ JADWAL AKAN DATANG ═══════════════════ -->
    <div class="sec" x-data="{ showAdd: false, showEdit: false, editData: {}, editFullday: true }">
        <div class="sec-head">
            <div class="sec-title">
                    ...
                    <div class="ev u">
                        ...
                        <div class="act-btns">
                            ...
                            <form method="POST" action="/jadwal-akan-datang/{{ $j->id }}"
                                onsubmit="return confirm('Hapus jadwal ini?')">
                                @csrf @method('DELETE')
                                <button type="submit" class="btn-del"><svg xmlns="http://www.w3.org/2000/svg" width="14" height="14"
        ...
        <!-- Modal Tambah -->
        <div x-show="showAdd" x-cloak class="modal-bg" @click.self="showAdd = false">
            <div class="modal" @click.stop x-data="{ fullday: true }">
                <div class="modal-head">
                ...
                <form method="POST" action="/jadwal-akan-datang">
                    ...
                    <div class="modal-body">
                        <div class="fg"><label>Judul Kegiatan</label><input type="text" name="judul" required
                                placeholder="Contoh: Seminar AI, Workshop"></div>
                        <div class="fg-row">
                        ...
                        <div class="fg-check">
                            <input type="checkbox" id="fd1" x-model="fullday">
                            <input type="hidden" name="is_fullday" :value="fullday ? 1 : 0">
                            <label for="fd1">Seharian (fullday)</label>
                        </div>
            ...
            <div class="modal" @click.stop>
                ...
                <form method="POST" :action="'/jadwal-akan-datang/' + editData.id">
                    ...
                    <div class="modal-body">
                        ...
                        <div class="fg-check">
                            <input type="checkbox" id="fd1e" x-model="editFullday">
                            <input type="hidden" name="is_fullday" :value="editFullday ? 1 : 0">
                            <label for="fd1e">Seharian (fullday)</label>
                        </div>
    ...
    <div class="sec" x-data="{ showAdd: false, showEdit: false, editData: {}, editFullday: true }">
        ...
        <div class="sec-body">
            @if($jadwalDadakan->count() > 0)
                ...
                @endforeach
            @else
                <div class="sec-empty">Belum ada jadwal dadakan</div>
            @endif
        </div>
        ...
        <div x-show="showAdd" x-cloak class="modal-bg" @click.self="showAdd = false">
            <div class="modal" @click.stop x-data="{ fullday: true }">
                ...
                <form method="POST" action="/jadwal-dadakan">
                    @csrf
                    <div class="modal-body">
                        <div class="fg"><label>Judul</label><input type="text" name="judul" required
                                placeholder="Contoh: Sakit, Izin, Tugas Mendadak"></div>
                        <div class="fg-check">
                            <input type="checkbox" id="fd2" x-model="fullday">
                            <input type="hidden" name="is_fullday" :value="fullday ? 1 : 0">
                            <label for="fd2">Seharian (fullday)</label>
                        </div>
                        <div class="fg" x-show="!fullday" x-cloak>
                            <label>Sampai Jam Berapa?</label>
                            <input type="time" name="jam_selesai">
                        </div>
                        <div class="fg"><label>Keterangan</label><textarea name="keterangan"
                                placeholder="Contoh: Dosen tidak hadir karena sakit"></textarea></div>
                    </div>
                    <div class="modal-foot">
                        <button type="button" class="btn-cancel" @click="showAdd = false">Batal</button>
                        <button type="submit" class="btn btn-dark"><i class="lucide-check" style="font-size:12px"></i>
                ...
                <form method="POST" :action="'/jadwal-dadakan/' + editData.id">
                    ...
                    <div class="modal-body">
                        <div class="fg"><label>Judul</label><input type="text" name="judul" required
                                x-model="editData.judul"></div>
                        <div class="fg-check">
                            <input type="checkbox" id="fd2e" x-model="editFullday">
                            <input type="hidden" name="is_fullday" :value="editFullday ? 1 : 0">
                            <label for="fd2e">Seharian (fullday)</label>
                        </div>
...
<script>
    ...
    // Attach ripple to all .btn elements on submit/click
